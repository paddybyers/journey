			...
			process.nextTick(function () {
				...
				that.dispatch(request, body || "", function (outcome) {
					...
					if(!headers["Server"] && !headers["server"])
						headers["Server"] = that.options.server || "journey/" + journey.version.join('.');

					if (outcome.body) {
						// only add Content-Type and Content-Length if not already present
						if(!headers["Content-Type"] && !headers["content-type"]) {
							if(Buffer.isBuffer(outcome.body)) {
								headers['Content-Length'] = outcome.body.length;
							} else if(typeof(outcome.body) == 'string') {
								headers['Content-Length'] = Buffer.byteLength(outcome.body);
							} else {
