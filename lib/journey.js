...
journey.Router.methods = ['GET', 'PUT', 'POST', 'DELETE', 'HEAD', 'OPTIONS'];
		...
		handle: function (request, body, callback) {
			...
			process.nextTick(function () {
				...
				that.dispatch(request, body || "", function (outcome) {
					if(!outcome.headers["Date"] && !outcome.headers["date"])
						outcome.headers["Date"] = new(Date)().toUTCString();
					if(!outcome.headers["Server"] && !outcome.headers["server"])
						outcome.headers["Server"] = that.options.server || "journey/" + journey.version.join('.');

					if (outcome.body) {
						if (typeof(outcome.body) !== 'string' && !outcome.headers["Content-Type"] && !outcome.headers["content-type"]) {
						...
						else if(Buffer.isBuffer(outcome.body))
